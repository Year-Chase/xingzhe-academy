...
interface DetailData {
  userId: string; nickname: string | null; birthYearMonth: string | null; age: number | null
...
const addTag = async () => {
  ...
  try {
    await post('/admin/crm/users/' + userId + '/tags', { tag: newTag.value.trim() })
    newTag.value = ''; MessagePlugin.success('标签已添加'); fetchDetail()
...
const removeTag = async (tagId: number) => {
  try {
    await del('/admin/crm/users/' + userId + '/tags/' + tagId)
    MessagePlugin.success('标签已删除'); fetchDetail()
...
const addNote = async () => {
  ...
  try {
    await post('/admin/crm/users/' + userId + '/notes', { note: newNote.value.trim() })
    newNote.value = ''; MessagePlugin.success('备注已添加'); fetchDetail()
  } catch (e: any) { MessagePlugin.error(e?.response?.data?.message || '添加失败') }
...
const removeNote = async (noteId: number) => {
  try {
    await del('/admin/crm/users/' + userId + '/notes/' + noteId)
    MessagePlugin.success('备注已删除'); fetchDetail()
  } catch (e: any) { MessagePlugin.error(e?.response?.data?.message || '删除失败') }
}

const showCert = (cert: any) => {
  certName.value = cert?.name || '证书'
  certImage.value = cert?.imageUrl || ''
  if (!certImage.value) { MessagePlugin.warning('暂无证书图片'); return }
  certDialog.value = true
}

const goUser = (uid: string) => {
  if (uid) router.push('/crm/users/' + uid)
}
...
const orderColumns = [
  { colKey: 'id', title: 'ID', width: 55 },
  { colKey: 'activityId', title: '活动', width: 55 },
...
const refundColumns = [
  { colKey: 'id', title: 'ID', width: 55 },
  { colKey: 'orderId', title: '订单', width: 55 },
  { colKey: 'amount', title: '金额', width: 85, cell: (_h: any, { row }: any) => yuan(row.amount) },
  { colKey: 'status', title: '状态', width: 60 },
  { colKey: 'reason', title: '原因', width: 110, cell: (_h: any, { row }: any) => row.reason || '-' },
  { colKey: 'createdAt', title: '时间', width: 130, cell: (_h: any, { row }: any) => fmt(row.createdAt) },
]
const invoiceColumns = [
  { colKey: 'id', title: 'ID', width: 55 },
  { colKey: 'title', title: '抬头', width: 150 },
  { colKey: 'amount', title: '金额', width: 85, cell: (_h: any, { row }: any) => yuan(row.amount) },
    ...
    <template v-if="data">
      ...
      <div style="background: #FFFFFF; border-radius: 12px; border: 1px solid #EDE9DF; padding: 24px; margin-bottom: 16px;">
        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
          <div>
            <h3 style="font-size: 20px; font-weight: 600; color: #18231E; margin: 0 0 4px 0;">用户 {{ data.userId }}</h3>
            <div style="color: #8A9288; font-size: 14px; line-height: 1.8;">
              <div>昵称: {{ data.nickname || '-' }}</div>
              <div>出生年月: {{ data.birthYearMonth || '-' }}</div>
              <div>年龄: {{ data.age !== null ? data.age : '-' }}</div>
              <div>注册时间: {{ fmt(data.registeredAt as string) }}</div>
              <div>最近登录: {{ fmt(data.lastLoginAt as string) }}</div>
            </div>
          </div>
      ...
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(120px, 1fr)); gap: 12px; margin-bottom: 16px;">
        <div style="background: #FFFFFF; border-radius: 10px; border: 1px solid #EDE9DF; padding: 16px;">
          <div style="color: #8A9288; font-size: 12px;">报名次数</div>
          <div style="color: #18231E; font-size: 24px; font-weight: 700;">{{ data.summary.registrationCount }}</div>
        </div>
        <div style="background: #FFFFFF; border-radius: 10px; border: 1px solid #EDE9DF; padding: 16px;">
          <div style="color: #8A9288; font-size: 12px;">订单数</div>
          <div style="color: #18231E; font-size: 24px; font-weight: 700;">{{ data.summary.orderCount }}</div>
        </div>
        <div style="background: #FFFFFF; border-radius: 10px; border: 1px solid #EDE9DF; padding: 16px;">
          <div style="color: #8A9288; font-size: 12px;">签到次数</div>
          <div style="color: #18231E; font-size: 24px; font-weight: 700;">{{ data.summary.checkedInCount }}</div>
        </div>
        <div style="background: #FFFFFF; border-radius: 10px; border: 1px solid #EDE9DF; padding: 16px;">
          <div style="color: #8A9288; font-size: 12px;">累计支付</div>
          <div style="color: #2E7D5A; font-size: 16px; font-weight: 700;">{{ yuan(data.summary.paidAmount) }}</div>
        </div>
        <div style="background: #FFFFFF; border-radius: 10px; border: 1px solid #EDE9DF; padding: 16px;">
          <div style="color: #8A9288; font-size: 12px;">累计退款</div>
          <div style="color: #B35B4B; font-size: 16px; font-weight: 700;">{{ yuan(data.summary.refundedAmount) }}</div>
        </div>
        <div style="background: #FFFFFF; border-radius: 10px; border: 1px solid #EDE9DF; padding: 16px;">
          <div style="color: #8A9288; font-size: 12px;">净收入</div>
          <div style="color: #18231E; font-size: 16px; font-weight: 700;">{{ yuan(data.summary.netAmount) }}</div>
        </div>
        <div style="background: #FFFFFF; border-radius: 10px; border: 1px solid #EDE9DF; padding: 16px;">
          <div style="color: #8A9288; font-size: 12px;">邀请注册</div>
          <div style="color: #18231E; font-size: 24px; font-weight: 700;">{{ data.summary.inviteRegisterCount ?? 0 }}</div>
        </div>
        <div style="background: #FFFFFF; border-radius: 10px; border: 1px solid #EDE9DF; padding: 16px;">
          <div style="color: #8A9288; font-size: 12px;">邀请活动</div>
          <div style="color: #18231E; font-size: 24px; font-weight: 700;">{{ data.summary.inviteActivityCount ?? 0 }}</div>
        </div>
      </div>
      ...
      <div style="background: #FFFFFF; border-radius: 12px; border: 1px solid #EDE9DF; padding: 20px; margin-bottom: 16px;">
        ...
        <div v-if="data.notes.length">
          <div v-for="note in data.notes" :key="note.id" style="padding: 10px 14px; background: #F7F6F2; border-radius: 8px; margin-bottom: 8px; display: flex; justify-content: space-between; align-items: flex-start;">
            <div style="flex: 1;">
              <div style="color: #18231E; font-size: 14px; white-space: pre-wrap;">{{ note.note }}</div>
              <div style="color: #8A9288; font-size: 12px; margin-top: 4px;">{{ fmt(note.createdAt) }}</div>
            </div>
      ...
      <div style="background: #FFFFFF; border-radius: 12px; border: 1px solid #EDE9DF; padding: 20px; margin-bottom: 16px;">
        ...
        <t-table v-if="data.registrations.length" :data="data.registrations" :columns="regColumns" row-key="id" size="small" hover stripe>
          ...
          <template #certificate="{ row }">
            <span v-if="row.certificate" style="color: #2E7D5A; cursor: pointer; text-decoration: underline;" @click="showCert(row.certificate)">{{ row.certificate.name || '证书' }}</span>
            <span v-else style="color: #8A9288;">-</span>
          </template>
          <template #inviter="{ row }">
            <span v-if="row.inviter" style="color: #2E7D5A; cursor: pointer;" @click="goUser(row.inviter.userId)">{{ row.inviter.name || row.inviter.userId }}</span>
            <span v-else style="color: #8A9288;">-</span>
          </template>
        </t-table>
      ...
      <div style="background: #FFFFFF; border-radius: 12px; border: 1px solid #EDE9DF; padding: 20px; margin-bottom: 16px;">
        ...
        <t-table v-if="data.orders.length" :data="data.orders" :columns="orderColumns" row-key="id" size="small" hover stripe>
          <template #status="{ row }">
            <span :style="{ color: statusColor(row.status), fontSize: '13px', fontWeight: 500 }">{{ orderStatusLabel(row.status) }}</span>
          </template>
        </t-table>
      ...
      <div style="background: #FFFFFF; border-radius: 12px; border: 1px solid #EDE9DF; padding: 20px; margin-bottom: 16px;">
        ...
        <t-table v-if="data.refunds.length" :data="data.refunds" :columns="refundColumns" row-key="id" size="small" hover stripe>
          <template #status="{ row }">
            <span :style="{ color: statusColor(row.status), fontSize: '13px', fontWeight: 500 }">{{ refundStatusLabel(row.status) }}</span>
          </template>
        </t-table>
      ...
      <div style="background: #FFFFFF; border-radius: 12px; border: 1px solid #EDE9DF; padding: 20px; margin-bottom: 16px;">
        ...
        <t-table v-if="data.invoices.length" :data="data.invoices" :columns="invoiceColumns" row-key="id" size="small" hover stripe>
          <template #status="{ row }">
            <span :style="{ color: statusColor(row.status), fontSize: '13px', fontWeight: 500 }">{{ invoiceStatusLabel(row.status) }}</span>
          </template>
        </t-table>
        <div v-else style="color: #8A9288; font-size: 13px; text-align: center; padding: 20px 0;">暂无发票记录</div>
      </div>

      <!-- Invite Records (registration) -->
      <div style="background: #FFFFFF; border-radius: 12px; border: 1px solid #EDE9DF; padding: 20px; margin-bottom: 16px;">
        ...
        <t-table v-if="data.inviteRecords.length" :data="data.inviteRecords" :columns="inviteRegColumns" row-key="id" size="small" hover stripe>
          <template #inviteeUserId="{ row }">
            <span style="color: #2E7D5A; cursor: pointer;" @click="goUser(row.inviteeUserId)">{{ row.inviteeName || row.inviteeUserId }}</span>
          </template>
        </t-table>
        <div v-else style="color: #8A9288; font-size: 13px; text-align: center; padding: 20px 0;">暂无邀请注册记录</div>
      </div>

      <!-- Activity Invite Records -->
      <div style="background: #FFFFFF; border-radius: 12px; border: 1px solid #EDE9DF; padding: 20px; margin-bottom: 16px;">
