...
- 当前仍为模拟支付/登记退款边界，不代表真实微信支付或真实原路退款。
- FULL 订单实付金额为全款；PREPAY 订单实付金额不包含未支付后付款。
- 已退款金额只累计成功登记退款。
- 可退款金额 = `max(实付金额 - 已成功退款金额, 0)`。
- 后付款完成或免除后，如报名未签到，会触发 POSTPAY 阶段二维码生成；已签到则不生成新签到二维码。
- 全额退款后，当前 ACTIVE 二维码应撤销，核销接口仍需实时检查全额退款状态。
