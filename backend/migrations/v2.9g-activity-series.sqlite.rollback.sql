-- V2.9G rollback for local SQLite.
-- Warning: activity.seriesId values and activity_series records are discarded.

PRAGMA foreign_keys = OFF;
BEGIN TRANSACTION;

DROP INDEX IF EXISTS idx_activity_series_id;

CREATE TABLE activity_v29g_rollback (
...
INSERT INTO activity_v29g_rollback (
  id, title, categoryId, slogan, province, description, location, city, startTime,
  endTime, registrationStartTime, registrationEndTime, capacity, coverImage,
  price, memberPrice, lifetimeMemberPrice, paymentMode, prepayAmount,
  remainingAmount, remainingPayDate, status, requiredUserInfoFields, groupQrType,
  groupQrImageUrl, groupQrTitle, groupQrDescription, memoryImages, memoryText,
  certificateTemplateId, imageUrls, contentBlocks, pricingRules, postpayDate,
  provinceName, provinceCode, cityName, cityCode, adcode, lng, lat, locationName,
  locationAddress, locationLat, locationLng, coordinateType, locationProvider,
  createdAt
)
SELECT
  id, title, categoryId, slogan, province, description, location, city, startTime,
...
DROP TABLE activity;
ALTER TABLE activity_v29g_rollback RENAME TO activity;
CREATE INDEX IF NOT EXISTS idx_activity_category_id ON activity (categoryId);
